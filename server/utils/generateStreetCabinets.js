    ...
    for (let i = 0; i < numCabinets; i++) {
      ...
      const newStreetCabinet = new Resource({
        ...
        resourceCharacteristic: [
          ...
          { name: "cabinet_heigth", value: `${faker.number.float({ min: 0.7, max: 2.0 }).toString()}m` },
          { name: "cabinet_width", value: `${faker.number.float({ min: 0.5, max: 1.5 }).toString()}m` },
          { name: "current_capacity_usage", value: `${faker.number.int({ min: 1, max: 100 }).toString()}%` },
          { name: "maximum_capacity", value: faker.number.int({ min: 400, max: 1500 }).toString() },
          { name: "network_type", value: faker.helpers.arrayElement(['DSL', 'FTTH','Koaxial'])},
          { name: "fiber_ready", value: faker.datatype.boolean() },
          { name: "power_backup", value: faker.datatype.boolean() },

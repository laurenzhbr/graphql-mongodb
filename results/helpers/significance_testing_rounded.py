...
def check_normality(data, alpha=0.05):
    ...
    return {
        "mean": round(mu, 2),
        "std_dev": round(std, 2),
        "shapiro_stat": round(shapiro_stat, 2),
        "shapiro_p": round(shapiro_p, 4),  # Shapiro-Wilk Test p-Wert mit vier Dezimalstellen
...
def perform_stat_test(rest_data, gql_data, metric):
    ...
    return test_type, round(p, 4)  # p-Wert auf 4 Nachkommastellen runden
